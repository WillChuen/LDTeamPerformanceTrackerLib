...
/// 团队通用监控适配插件
@objc(LDTeamCommonTracker) public class LDTeamCommonTracker: NSObject, LDTeamTrackerPlugin {
    /// 开始一个监控
    public func start(name: String, operation: String) {
        print("CommonTracker Start - Name: \(name), Operation: \(operation)")
    }
    /// 结束一个监控
    public func finish(name: String, status: LDTeamTrackerStatus) {
